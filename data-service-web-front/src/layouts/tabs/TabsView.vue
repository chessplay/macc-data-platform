  ...
  methods: {
      ...
      if (cachedTabsStr) {
        try {
          ...
          if (cachedTabs.length > 0) {
            //排除当前路由不存在得缓存
            const routers = this.getRoutesMap();
            const cachedTabsFilter = cachedTabs.filter((item) => {
              return routers.indexOf(item.fullPath) > -1;
            });
            this.pageList = cachedTabsFilter;
          }
        } catch (e) {
    ...
    getRoutesMap() {
      ...
      for (let i = 0; i < routers.length; i++) {
        routersmap.push(routers[i].path);
      }
      return routersmap;
    },
